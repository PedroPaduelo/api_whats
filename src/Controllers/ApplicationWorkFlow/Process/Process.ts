...
export async function CreateProcess(request, response) {
...
export async function ListProcess(request, response) {
  try {
    const result = await connection.select("*").from(table)
    return response.json(result); 
  } catch (error) {
...
export async function UpdateProcess(request, response) { 
