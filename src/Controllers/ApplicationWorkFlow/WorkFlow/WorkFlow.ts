...
export async function CreateWorkFlow(request, response) {
...
export async function ListWorkFlow(request, response) {
  try {
    const result = await connection.select("*").from(table)
    return response.json(result); 
  } catch (error) {
...
export async function UpdateWorkFlow(request, response) { 
